...
function accuracyColor(acc: number | null): string {
  if (acc == null) return "";
  if (acc >= 93) return "text-emerald-400 bg-emerald-900/30";
  if (acc >= 85) return "text-amber-400 bg-amber-900/30";
...
export default function ModelPerformancePage() {
  ...
  if (error) return (
    <div className="bg-red-900/30 border border-red-700/50 rounded-lg p-6 text-center my-8">
      <p className="text-red-400 font-medium">Failed to load model data</p>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Accuracy = 100% minus forecast error (MAPE)</p>
        </div>
        <div className="flex rounded-lg border border-slate-600 overflow-hidden">
          {(["all", "CC", "DC"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-2 text-sm font-medium transition-colors ${filter === f ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-5 text-center">
          <p className="text-sm text-slate-400 mb-1">Overall Median Accuracy</p>
          <p className="text-3xl font-bold text-emerald-400">{overallAcc ? overallAcc + "%" : "—"}</p>
          <p className="text-xs text-slate-500 mt-1">across all bank models</p>
        </div>
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-5 text-center">
          <p className="text-sm text-slate-400 mb-1">Credit Card Models</p>
          <p className="text-3xl font-bold text-blue-400">{ccMedianAcc ? ccMedianAcc + "%" : "—"}</p>
          <p className="text-xs text-slate-500 mt-1">median accuracy (10 banks)</p>
        </div>
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-5 text-center">
          <p className="text-sm text-slate-400 mb-1">Debit Card Models</p>
          <p className="text-3xl font-bold text-blue-400">{dcMedianAcc ? dcMedianAcc + "%" : "—"}</p>
          <p className="text-xs text-slate-500 mt-1">median accuracy (15 banks)</p>
        </div>
      </div>

      {bankModels.length > 0 && (
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6 mb-6">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Bank-Level Models</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {bankModels.map((m) => (
                  <tr key={`${m.bank_name}-${m.card_type}`} className="border-b border-slate-700/50 last:border-0 hover:bg-slate-700/30">
                    <td className="py-3 font-medium text-slate-200">{m.bank_name}</td>
                    <td className="py-3 text-slate-400">{m.card_type === "CC" ? "Credit" : "Debit"}</td>
                    <td className="py-3 text-slate-500">{m.model_type}</td>
                    ...
                    <td className="py-3 text-right text-slate-500">{m.last_trained ? new Date(m.last_trained).toLocaleDateString("en-IN") : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {aggModels.length > 0 && (
        <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 p-6">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Aggregate Models</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700 text-left text-slate-400">
              ...
              <tbody>
                {aggModels.map((m) => (
                  <tr key={m.metric} className="border-b border-slate-700/50 last:border-0 hover:bg-slate-700/30">
                    <td className="py-3 font-medium text-slate-200">{m.metric?.replace(/_/g, " ")}</td>
                    <td className="py-3 text-slate-500">{m.model_type}</td>
                    <td className="py-3 text-right"><span className={`px-2 py-0.5 rounded text-xs font-medium ${accuracyColor(accuracyFromMape(m.cv_mape))}`}>{accuracyBadge(m.cv_mape)}</span></td>
                    <td className="py-3 text-right text-slate-500">{m.last_trained ? new Date(m.last_trained).toLocaleDateString("en-IN") : "—"}</td>
                  </tr>
      ...
      {models.length === 0 && (
        <p className="text-slate-500 text-center py-8">No model metadata available yet.</p>
      )}
    </div>
  );
}
